...
export const HighPriority: React.FC<{ tasks: ITask[] | undefined }> = ({
  tasks,
}) => {
  ...
  return (
    ...
    >
      {tasks?.map((item) => ( item.priority === "high_priority" &&
        <div key={item.id} className="bg-[#fd9fa1] rounded-md p-2 w-full">
          <h2 className="font-semibold">{item.title}</h2>
          <h2 className="text-black/70">{item.description}</h2>
          <div className="flex justify-between">
          <p className="bg-red-400 text-sm border w-fit rounded-sm px-2 text-white mt-2">
            {item.priority}
          </p>
