...
export default function Alarm({
  ...
  return (
    <>
      <div className={`flex items-center m-5 border-2 ${isOff ? "bg-gray-300/75" : "bg-white/95"} border-white px-2  rounded-md border-r-2 shadow-lg shadow-blueApp_1`}>
        ...
        <div className="flex gap-x-3 py-4 max-sm:px-3 flex-wrap max-sm:justify-center max-sm:gap-y-1 max-sm:py-2  pr-4 font-bold pl-6">
          <button
            ...
            className={`px-2 py-1 max-sm:text-sm border-2 border-white rounded-md hover:text-white shadow-blue-300/60 shadow-lg ${isOff ? "bg-blueApp_2/30 hover:bg-blueApp_2/40" : "bg-blueApp_2 hover:bg-blueApp_2/50"}`}
          >
            Edit
          </button>
          <button
            onClick={() => showModalChange("delete")}
            className={`px-2 py-1 max-sm:text-sm border-2 border-white rounded-md hover:text-white shadow-blue-300/60 shadow-lg ${isOff ? "bg-redApp/60 hover:bg-redApp/30" : "bg-redApp hover:bg-redApp/50"}`}
          >
            Delete
          </button>
        </div>
