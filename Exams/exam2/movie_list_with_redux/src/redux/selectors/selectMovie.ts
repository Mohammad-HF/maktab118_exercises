...
export const selectSortMovies = createSelector(selectMovie, (movie) => {
    if(movie.list.length > 1 && movie.sort !== ""){
        ...
        ? {list : movie.list.sort((it1, it2) =>
          it1.movieName.localeCompare(it2.movieName)
        ), sort : movie.sort}
        : movie.sort === "byGenre"
        ? {list : movie.list.sort((it1, it2) => it1.genre.localeCompare(it2.genre)) , sort : movie.sort}
        : movie.sort === "byScore"
        ? {list : movie.list.sort((it1, it2) => Number(it1.score) - Number(it2.score)) , sort : movie.sort}
